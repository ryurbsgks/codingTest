...
function solution(n, build_frame) {
  ...
  const checkPillar = (answer, x, y) => {
    if (y === 0) {
      return true;
    } else if (answer.find( ([a, b, fr]) => a === x && b === y - 1 && fr === 0)) {
      return true;
    } else if (answer.find( ([a, b, fr]) => a === x && b === y && fr === 1)) {
      return true;
    } else if (answer.find( ([a, b, fr]) => a === x - 1 && b === y && fr === 1)) {
      return true;
    }
  ...
  const checkPlate = (answer, x, y) => {
    if (answer.find( ([a, b, fr]) => a === x && b === y - 1 && fr === 0)) {
      return true;
    } else if (answer.find( ([a, b, fr]) => a === x + 1 && b === y - 1 && fr === 0)) {
      return true;
    } else if (answer.find( ([a, b, fr]) => a === x + 1 && b === y && fr === 1) && answer.find( ([a, b, fr]) => a === x - 1 && b === y && fr === 1)) {
      return true;
    }
  ...
  const buildFrame = (answer, x, y, frame) => {
    if (frame) {
      if (checkPlate(answer, x, y)) {
        answer.push([x, y, frame]);
      }
    } else {
  ...
  const destroyFrame = (answer, x, y, frame) => {
    
    let copy = answer.slice();
    let index = answer.findIndex( ([a, b, fr]) => a === x && b === y && fr === frame);
    ...
    for (let el of copy) {
      ...
      if (fr) {
        if (!checkPlate(copy, xpos, ypos)) {
          return;
        }
      } else {
        if (!checkPillar(copy, xpos, ypos)) {
          return;
        }
      }
    }
  ...
  for (let el of build_frame) {

    let [x, y, fr, isInstall] = el;
    
    if (isInstall) {
      buildFrame(answer, x, y, fr);
    } else {
      destroyFrame(answer, x, y, fr);
    }
  }
