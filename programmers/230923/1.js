...
function solution(sales, links) {
  ...
  const calculate = (obj, sales, newObj, idx) => {
    
    let children = obj[idx];
    ...
    children.map( (el) => {
      if (!newObj[el]) {
        if (obj[el]) {
          calculate(obj, sales, newObj, el); 
        } else {
          newObj[el] = [0, sales[el - 1]];
        }
      }
    });
    
    newObj[idx][0] = children.reduce( (acc, cur) => {
      if (newObj[cur][0] < newObj[cur][1]) {
        return acc + newObj[cur][0];
      } else {
        checkGroup = true;

        return acc + newObj[cur][1];
      }
    }, 0);
    
    newObj[idx][1] = newObj[idx][0] + sales[idx - 1];
    ...
    if (!checkGroup) {
      ...
      children.map( (el) => {
        ...
        let tempOffset = newObj[el][1] - newObj[el][0];
  ...
  links.map( (el) => {
    ...
    let [leader, member] = el;

    if (obj[leader]) {
      obj[leader].push(member);
    } else {
      obj[leader] = [member];
    }
...
console.log(solution([14, 17, 15, 18, 19, 14, 13, 16, 28, 17], [[10, 8], [1, 9], [9, 7], [5, 4], [1, 5], [5, 10], [10, 6], [1, 3], [10, 2]])) // 44
console.log(solution([5, 6, 5, 3, 4], [[2,3], [1,4], [2,5], [1,2]])) // 6
console.log(solution([5, 6, 5, 1, 4], [[2,3], [1,4], [2,5], [1,2]])) // 5
console.log(solution([10, 10, 1, 1], [[3,2], [4,3], [1,4]])) // 2