...
function solution(n, paths, gates, summits) {
  ...
  let newArr = new Array(n + 1).fill(Infinity);
  let q = gates;
  ...
  summits.map( (el) => arr[el] = []);
  gates.map( (el) => newArr[el] = -1);
  ...
  while (q.length > 0) {
    ...
    while (q.length > 0) {
      ...
      for (let [w, v] of arr[qv]) {

        let maxV = Math.max(newArr[qv], w);
  ...
  let res = summits.map( (el) => [el, newArr[el]]).sort( (a, b) => {
    if (a[1] === b[1]) {
      return a[0] - b[0];
    }
...
console.log(solution(6, [[1, 2, 3], [2, 3, 5], [2, 4, 2], [2, 5, 4], [3, 4, 4], [4, 5, 3], [4, 6, 1], [5, 6, 1]], [1, 3], [5])) // [5, 3]
