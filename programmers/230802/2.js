...
function solution(m, n, startX, startY, balls) {
  ...
  const distance = (m, n, x1, y1, x2, y2) => {
    ...
    return arr.reduce( (acc, [x, y]) => Math.min(acc, Math.pow(x - x2, 2) + Math.pow(y - y2, 2)), Math.pow(m, 2) + Math.pow(n, 2));
  };

  answer = balls.map( (el) => distance(m, n, startX, startY, el[0], el[1]));
