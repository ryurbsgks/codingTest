...
function solution(user_id, banned_id) {
  ...
  let arr = new Array(user_id.length).fill(false);
  let regExp = banned_id.map( (el) => new RegExp(`^${el.replaceAll("*", ".")}$`));
  let set = new Set();

  const dfs = (index, newArr) => {
    if (index === banned_id.length) {
      set.add(newArr.sort().join(","));
    } else {
      for (let i = 0; i < user_id.length; i++) {
        ...
        if (user_id[i].match(regExp[index])) {
          arr[i] = true;
          dfs(index + 1, [...newArr, user_id[i]]);
          arr[i] = false;
        }
      }
    }
