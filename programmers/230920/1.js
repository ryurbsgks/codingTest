...
function solution(n, path, order) {
  ...
  let nodes = Array.from({ length: n }, () => new Node([], 0, false, 0));
  let start = nodes[0];
  let stack = [];
  ...
  const visit = (node, nodes) => {
    ...
    let priorNode = nodes[current.prior];
  ...
  for (let el of path) {
    nodes[el[0]].edges.push(el[1]);
    nodes[el[1]].edges.push(el[0]);
  }

  for (let el of order) {
    nodes[el[1]].prior = el[0];
  }
  ...
  start.visited = true;
  start.edges.map( (el) => stack.push(el));

  while (stack.length !== 0) {

    let node = stack.pop();
    let availables = visit(node, nodes);

    availables.map( (el) => stack.push(el));
...
console.log(solution(9, [[0,1],[0,3],[0,7],[8,1],[3,6],[1,2],[4,7],[7,5]], [[8,5],[6,7],[4,1]])) // true
console.log(solution(9, [[8,1],[0,1],[1,2],[0,7],[4,7],[0,3],[7,5],[3,6]], [[4,1],[5,2]])) // true
console.log(solution(9, [[0,1],[0,3],[0,7],[8,1],[3,6],[1,2],[4,7],[7,5]], [[4,1],[8,7],[6,5]])) // false