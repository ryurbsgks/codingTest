...
function solution(X, Y) {
  ...
  for (let i = 0; i < 10; i++) {
    answer = answer + i.toString().repeat(Math.min(arrX.filter( (el) => Number(el) === i).length, arrY.filter( (el) => Number(el) === i).length));
  }

  if (answer === "") {
    answer = "-1";
  } else if (answer.length === answer.split("").filter( (el) => el === "0").length) {
    answer = "0";
  } else {
